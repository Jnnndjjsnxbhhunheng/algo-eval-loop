...
**⚠️ 平台期策略：连续追平 best 未超越 → 升级改动幅度**

`skill_loop.py log` 会自动检测平台期：连续 3 轮 score 追平 best 但 decision=revert，打印警告。收到警告时必须：
1. 停止在同一层面继续微调
2. 升级到结构级改动（重组处理步骤、调整数据流、增删中间环节）
3. 结构级改动仍须满足简洁性原则和 overfit 约束
